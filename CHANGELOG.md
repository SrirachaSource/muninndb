...
### Added
- Build receipt: the server can now name the source revision it was compiled from. `GET /version` (and `/mcp/version`) return `{version, revision, short_revision, commit_time, modified, go_version}`; the same block is included in `GET /mcp/health` under `build` and in the `muninn_status` tool result, and `muninndb-server version` prints it. Both endpoints are unauthenticated and make no engine call, so they answer while the database is degraded. Previously a deployed binary carried no way to identify its source — operators inferred the running revision by comparing commit timestamps against deploy timestamps, which was wrong at least once. The data required no new build plumbing: the Go toolchain already stamps VCS revision, commit time and a dirty-tree flag into every binary built from a version-controlled tree, and that stamping survives both the `git clone --depth 1` and the `-ldflags="-s -w"` used by the deploy image. It was present and unread — greppable from the binary on the box, but never served by the process itself, so identifying a build required container access. Note `commit_time` is the COMMIT timestamp, not a build timestamp: the toolchain does not stamp when compilation happened (that would break reproducible builds), and a commit can ship in an image built days later, so the field is named for what it actually is. Fields report `"unknown"` rather than an empty string when a stamp is absent (e.g. a `-buildvcs=false` or tarball build), and a binary built from a modified tree is reported as `modified: true`.
