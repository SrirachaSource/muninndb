...
func (a *mcpEngineAdapter) Evolve(ctx context.Context, vault, oldID, newContent, reason, concept string, embedding []float32) (*WriteResult, error) {
	id, err := a.eng.EvolveWithConcept(ctx, vault, oldID, newContent, reason, concept, embedding)
	if err != nil {
		return nil, err
	}
